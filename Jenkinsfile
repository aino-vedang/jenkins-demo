    ...
    stage('Script test') {
      steps {
            sh '''#!/bin/bash
                ...
                sed -i "s,.*ProxyPass /tool1/.*,ProxyPass /tool1/ http://localhost:$freePortJetty/," /etc/test-apache.conf
                sed -i "s,.*ProxyPassReverse /tool1/.*,ProxyPassReverse /tool1/ http://localhost:$freePortJetty/," /etc/test-apache.conf
